---
layout: post
title:  "Translation trade-offs"
slug: translation-philosophy
date: 2026-03-08
comments: true
excerpt: |
  <figure>
    <img src="https://images.unsplash.com/photo-1677641905377-e3f6087b8b7a" alt="A display case with a scale, cups, and other items" style="width: 100%; height: 280px; object-fit: cover;" />
  </figure>
---

# 1. Translation?

Perfect translation is impossible. Words for the same object, feeling, or process have slightly different connotations, melodies, and textures. Different grammars lead to different rhythms. Above all, the mindset of a native speaker of one language cannot be directly mapped to that of another.

English "bread" is not German „Brot“. 

<figure>
  <div class="image-pair" style="display: flex; gap: 12px; align-items: stretch;">
    <img
      src="https://images.unsplash.com/photo-1584796101179-52cfea2e6f52"
      alt="A loaf of bread on a wooden chopping board"
      style="flex: 1; height: 240px; object-fit: cover;"
    />
    <img
      src="https://images.unsplash.com/photo-1657023102376-b575acc901d3"
      alt="A close-up of bread"
      style="flex: 1; height: 240px; object-fit: cover;"
    />
  </div>
</figure>

{:refdef: style="text-align: center;"}
*(source: Unsplash [[1]](https://unsplash.com/photos/bread-on-brown-wooden-chopping-board-zYsB2mezSnA), [[2]](https://unsplash.com/photos/a-close-up-of-some-food-uVxAroQfGPM))*
{: refdef}

# 2. Choices & bounds

So translating anything requires choosing what must be kept and what can be dropped.

The choices are especially difficult in poetic translation, where the meter and the rhyming pattern bind the translator's hands.

The bounds are illusory, of course. If our translator values precision above all else, the meter can be broken and the rhymes can be ignored.

Each translator decides themselves. Here, I want to make my priorities explicit, both to allow me to better understand them and to let you, dear reader, know what to expect from my translations.

# 3. Dimensions

Along which dimensions can trade-offs be made?

#### Spirit

For me, **fidelity to the spirit of the original** is paramount. A tragedy must not become a farce, nor a farce - a tragedy. Word choices matter - an imprecisely chosen word that may technically have the same translation in the dictionary can be very ill-fitting to the context, breaking the spell that the author casts. I aim never to allow this, no matter what else must be sacrificed.

#### Meaning

What can be sacrificed, then? Let's start with what translators who are poets themselves sacrifice most often but I consider only a little less vital than the spirit - the **fidelity to the meaning of the original**. I find it good and proper to faithfully translate what the author wrote and to use metaphors and turns of phrase close to those of the original, even if this results in worse rhymes and meter. 

Translators who are poets often take huge liberties with the text in order to arrive at a poem that scans well. Kipling's [The  Gipsy Trail](https://www.kiplingsociety.co.uk/poem/poems_gipsytrail.htm) has a pleasantly sounding [Russian translation](https://www.askbooka.ru/stihi/redyard-kipling/za-cyganskoy-zvezdoy.html), whose meaning, nonetheless, wildly diverges by the **third line**! 

A translation of a poem by another poet is often akin to a movie adaptation of a book, with changes upon changes upon changes. The result can be enjoyed on its own, but not side by side with the original (at least, not by me).

#### Meter

Then we have the **meter**, the sequence of stressed and unstressed syllables in a line. It matters, but some liberties with extra/missing syllables could be taken - especially when translating songs, since they can be sung to the same melody as long as the meter is close enough.

#### Rhymes

**Rhymes** are quite important for poetry (fight me, post-modernists!). Still, I often cannot find a rhyme that is both technically perfect and allows the spirit & the meaning to be preserved. If so, I go for rhymes that are trivial (e.g., rhyming verbs in Russian is super easy and therefore unsatisfying) or "bad" (where words don't sound too much alike; of the Geralt of Rivia [it was said](https://en.wikipedia.org/wiki/Sword_of_Destiny) that he "can only rhyme “rear” with “beer”" (in the Russian translation, "попа и жопа" [were used](https://www.livelib.ru/quote/114727-vedmak-mech-prednaznacheniya-andzhej-sapkovskij), which is objectively much funnier)). This ain't pretty, but it can be prettier than the alternatives.

#### Naturalness

Finally, an interesting dimension I defined for myself is **"Could a native speaker have written the translated song"?** Some translations are clearly translations, and that is fine. However, a perfect translation does not register as a translation at all - it seems like an excellent original poem. I'm sure that most Russian speakers don't know that the translation of The Gipsy Trail mentioned above is a translation rather than an original [song](https://www.youtube.com/watch?v=O7lYXuJMwYo). I find this to be desirable, but not as desirable as preserving the meaning (which I already complained about - but oh, how it irks me!).

# 4. Priorities

<figure>
  <img
    src="https://images.unsplash.com/photo-1677641905377-e3f6087b8b7a"
    alt="A display case with a scale, cups, and other items"
    style="width: 100%; height: 290px; object-fit: cover;"
  />
</figure>

{:refdef: style="text-align: center;"}
*(source: Unsplash [[3]](https://unsplash.com/photos/a-display-case-with-a-scale-cups-and-other-items-8Y2JjK2XVmU))*
{: refdef}

That rounds out the dimensions I'm aware of! I went through them in the order I prioritize them: **spirit > meaning > meter > rhymes > naturalness**

Of course, from multi-objective optimization we know that making trade-offs ain't easy, and sometimes large gains on lower-ranked dimensions are worth small losses on higher-ranked ones.

Except the spirit. I swear out loud whenever I see a translator butcher that. "Niet doen!" as the Dutch say. Don't do it! Just don't!

# 5. Living words

Trading off these dimensions is an imprecise and difficult art that I'm certainly not an authority on. Still, I find the process & the result rewarding, and I recommend that you, dear reader, try your hand at it too!

After all, songs & poetry span languages and predate writing, with imprecision tied to their nature as activities done by us, imperfect humans. Songs change as they travel through space & time - a translation is but one such change, and one that doesn't have to be perfect to be enjoyable. 

... This is what I tell myself when I realize that after working on a translation for weeks or months, I still cannot make it perfect. I recall that trade-offs are unavoidable and that letting go is healthy. So I declare the work done and move on.

Sometimes, I do go back. Looking with fresh eyes can be enlightening and my skills improve over time. So I tinker with the old translations. If so, I keep the date of the original version (however different it may have been), as these translations are a part of my life story, landmarks that help me remember my previous path on the river of time. 

Still, I like viewing these words as living, potentially changing when I change. The meticulous part of me struggles against the imprecision of all this - and so to pacify it, I'm explicitly discussing this here. Thus, beware: as words are wind, so are dates.

In the end, I hope that you, dear reader, will enjoy reading (or maybe reciting or singing) my translations as much as I enjoyed crafting them!

{% include comments.html %}
